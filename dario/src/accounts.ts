...
import { resolveAccountProxy } from './socks-bridge.js';
...
export interface AccountCredentials {
  ...
  accountUuid: string;
  /**
   * Optional per-account upstream egress proxy (http/https/socks5). When set,
   * this account's inference AND token-refresh traffic route through it so a
   * pooled account keeps a stable outbound IP. socks5 is tunneled via the
   * loopback bridge in src/socks-bridge.ts (Bun fetch can't do socks directly).
   * Empty / absent → direct (or the global --upstream-proxy if configured).
   */
  proxy?: string;
}
...
async function doRefreshAccountToken(creds: AccountCredentials): Promise<AccountCredentials> {
  ...
  // Route the refresh through the account's own proxy so the control-plane
  // call shares the IP the inference traffic uses. Best-effort: a bridge
  // failure here just means a direct refresh, not a hard error.
  let proxy: string | undefined;
  try {
    proxy = await resolveAccountProxy(creds.proxy);
  } catch {
    proxy = undefined;
  }
